 ...
 * ----------------------------------------
 * After migrating `workflow-runtime-artifacts.controller.ts` from
 ...
 */
describe('WorkflowRuntimeArtifactsController — PermissionsGuard integration', () => {
  const lifecycleTools = {
    createArtifact: vi.fn(),
    listArtifacts: vi.fn(),
    saveScriptAsArtifact: vi.fn(),
  ...
  function buildCtx(
    controller: WorkflowRuntimeArtifactsController,
    handler: (...args: unknown[]) => unknown,
    user: unknown,
  ) {
    return {
      getHandler: () => handler,
      getClass: () => WorkflowRuntimeArtifactsController,
      switchToHttp: () => ({
  ...
  const handlerPermissionExpectations: Array<{
    name: keyof WorkflowRuntimeArtifactsController;
    permission: string;
  }> = [
    { name: 'createArtifact', permission: 'memory:create' },
    { name: 'listArtifacts', permission: 'memory:read' },
    { name: 'saveScriptAsArtifact', permission: 'memory:create' },
    { name: 'listArtifactFiles', permission: 'memory:read' },
    { name: 'upsertArtifactFile', permission: 'memory:update' },
    { name: 'deleteArtifactFile', permission: 'memory:manage' },
  ];
  ...
  for (const { name, permission } of handlerPermissionExpectations) {
    it(`declares ${permission} as the required permission for ${name}`, () => {
      ...
      // documented permission (or `memory:manage` for the lifecycle
      // handler that deletes an artifact file).
      const controller = new WorkflowRuntimeArtifactsController(lifecycleTools);
      const reflector = new Reflector();
  ...
  it('allows a user that holds the required permission', async () => {
    ...
    const handler = controller.createArtifact as (
      ...args: unknown[]
    ) => unknown;
    ...
    expect(enforcement.getMode).toHaveBeenCalledWith('memory');
  ...
  it('records a would-deny audit under staged-enforcement (audit mode) but still allows', async () => {
    ...
    const handler = controller.deleteArtifactFile as (
      ...args: unknown[]
    ) => unknown;
    const result = await guard.canActivate(
      buildCtx(controller, handler, { userId: 'agent:run-1:job-1' }),
