import { Reflector } from '@nestjs/core';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { PermissionsGuard } from '../../auth/authorization/permissions.guard';
import { REQUIRED_PERMISSION_KEY } from '../../auth/authorization/require-permission.decorator';
import { WorkflowRuntimeInternalToolCallbacksController } from './workflow-runtime-internal-tool-callbacks.controller';
import type { WorkflowRuntimeToolsService } from './workflow-runtime-tools.service';

describe('WorkflowRuntimeInternalToolCallbacksController', () => {
  it('preserves loose callback payload fields while separating runtime context', async () => {
    const executeInternalTool = vi.fn().mockResolvedValue({ results: [] });
    const runtimeTools = {
      executeInternalTool,
    } as unknown as WorkflowRuntimeToolsService;
    const controller = new WorkflowRuntimeInternalToolCallbacksController(
      runtimeTools,
    );

    await controller.searchWorkflows(
      { user: { userId: 'developer-1', roles: ['Developer'] } },
      {
        workflow_run_id: 'run-1',
        query: 'dispatch',
      },
    );

    expect(executeInternalTool).toHaveBeenCalledWith({
      name: 'search_workflows',
      payload: { query: 'dispatch' },
      workflow_run_id: 'run-1',
      job_id: undefined,
      user: { userId: 'developer-1', roles: ['Developer'] },
    });
  });
});

/**
 * Authorization migration regression tests
 * ----------------------------------------
 * After migrating
 * `workflow-runtime-internal-tool-callbacks.controller.ts` from the
 * legacy role-based guard class to `PermissionsGuard` +
 * `RequirePermission`, exercise the real PermissionsGuard against
 * the migrated handler metadata. This pins three behaviors for the
 * audit record:
 *   (a) a permission-bearing user is allowed,
 *   (b) a user lacking the required permission is denied,
 *   (c) the audit service records the denial.
 */
describe('WorkflowRuntimeInternalToolCallbacksController — PermissionsGuard integration', () => {
  const runtimeTools = {
    executeInternalTool: vi.fn(),
  } as unknown as WorkflowRuntimeToolsService;

  function buildGuard(
    authzResult: boolean,
    mode: 'audit' | 'enforce' | 'warn' = 'enforce',
  ) {
    const reflector = new Reflector();
    const authz = {
      can: vi.fn().mockResolvedValue(authzResult),
    } as unknown as ConstructorParameters<typeof PermissionsGuard>[1];
    const enforcement = {
      getMode: vi.fn().mockResolvedValue(mode),
    } as unknown as ConstructorParameters<typeof PermissionsGuard>[2];
    const authzAudit = {
      recordDenial: vi.fn().mockResolvedValue(undefined),
    } as unknown as NonNullable<
      ConstructorParameters<typeof PermissionsGuard>[3]
    >;
    return {
      reflector,
      authz,
      enforcement,
      authzAudit,
      guard: new PermissionsGuard(reflector, authz, enforcement, authzAudit),
    };
  }

  function buildCtx(
    controller: WorkflowRuntimeInternalToolCallbacksController,
    handler: (...args: unknown[]) => unknown,
    user: unknown,
  ) {
    return {
      getHandler: () => handler,
      getClass: () => WorkflowRuntimeInternalToolCallbacksController,
      switchToHttp: () => ({
        getRequest: () => ({ user, params: {}, query: {}, body: {} }),
      }),
    } as unknown as Parameters<PermissionsGuard['canActivate']>[0];
  }

  beforeEach(() => {
    vi.clearAllMocks();
  });

  // ─── Per-handler @RequirePermission metadata ──────────────────────────

  const handlerPermissionExpectations: Array<{
    name: keyof WorkflowRuntimeInternalToolCallbacksController;
    permission: string;
  }> = [
    { name: 'searchWorkflows', permission: 'workflows:read' },
    { name: 'readWorkflowSummary', permission: 'workflows:read' },
    { name: 'searchSkills', permission: 'skills:read' },
    { name: 'readSkillManifest', permission: 'skills:read' },
    { name: 'searchPlaybooks', permission: 'workflows:read' },
    { name: 'readPlaybook', permission: 'workflows:read' },
    { name: 'listPendingCandidates', permission: 'skills:read' },
    { name: 'promoteCandidate', permission: 'skills:manage' },
    { name: 'rejectCandidate', permission: 'skills:manage' },
    { name: 'createSkillProposal', permission: 'skills:create' },
    { name: 'suggestSkillAssignment', permission: 'skills:create' },
    { name: 'materializeSkill', permission: 'skills:create' },
    { name: 'materializeSkillUpdate', permission: 'skills:update' },
  ];

  for (const { name, permission } of handlerPermissionExpectations) {
    it(`declares ${permission} as the required permission for ${name}`, () => {
      // Pin the migration: the legacy Admin/Developer/Agent role-list
      // decorator is gone and the handler now requires the agent's
      // documented permission for the targeted resource, with
      // `skills:manage` used for the lifecycle promote/reject
      // transitions.
      const controller = new WorkflowRuntimeInternalToolCallbacksController(
        runtimeTools,
      );
      const reflector = new Reflector();
      const handler = controller[name] as (...args: unknown[]) => unknown;
      const observed = reflector.get(REQUIRED_PERMISSION_KEY, handler);
      expect(observed).toBe(permission);
    });
  }

  // ─── Permission-bearing user is allowed ───────────────────────────────

  it('allows a user that holds the required permission', async () => {
    const { authz, enforcement, authzAudit, guard } = buildGuard(true);
    const controller = new WorkflowRuntimeInternalToolCallbacksController(
      runtimeTools,
    );

    const handler = controller.searchWorkflows as (
      ...args: unknown[]
    ) => unknown;
    const result = await guard.canActivate(
      buildCtx(controller, handler, { userId: 'agent:run-1:job-1' }),
    );

    expect(result).toBe(true);
    expect(authz.can).toHaveBeenCalledWith(
      'agent:run-1:job-1',
      'workflows:read',
      expect.any(String),
      undefined,
    );
    expect(enforcement.getMode).toHaveBeenCalledWith('workflows');
    expect(authzAudit.recordDenial).not.toHaveBeenCalled();
  });

  // ─── User lacking permission is denied ────────────────────────────────

  it('denies a user lacking the required permission', async () => {
    const { authz, guard } = buildGuard(false, 'enforce');
    const controller = new WorkflowRuntimeInternalToolCallbacksController(
      runtimeTools,
    );

    const handler = controller.promoteCandidate as (
      ...args: unknown[]
    ) => unknown;
    const result = await guard.canActivate(
      buildCtx(controller, handler, { userId: 'agent:run-1:job-1' }),
    );

    expect(result).toBe(false);
    expect(authz.can).toHaveBeenCalledWith(
      'agent:run-1:job-1',
      'skills:manage',
      expect.any(String),
      undefined,
    );
  });

  // ─── Audit service records the denial ────────────────────────────────

  it('records a denial via AuthorizationAuditService for every handler', async () => {
    const { authzAudit, guard } = buildGuard(false, 'enforce');
    const controller = new WorkflowRuntimeInternalToolCallbacksController(
      runtimeTools,
    );

    for (const { name, permission } of handlerPermissionExpectations) {
      const handler = controller[name] as (...args: unknown[]) => unknown;
      await guard.canActivate(
        buildCtx(controller, handler, { userId: 'agent:run-1:job-1' }),
      );

      expect(authzAudit.recordDenial).toHaveBeenCalledWith(
        expect.objectContaining({
          actorId: 'agent:run-1:job-1',
          requiredPermission: permission,
          enforcementMode: 'enforce',
        }),
      );
    }
  });

  it('records a would-deny audit under staged-enforcement (audit mode) but still allows', async () => {
    const { authzAudit, guard } = buildGuard(false, 'audit');
    const controller = new WorkflowRuntimeInternalToolCallbacksController(
      runtimeTools,
    );

    const handler = controller.materializeSkillUpdate as (
      ...args: unknown[]
    ) => unknown;
    const result = await guard.canActivate(
      buildCtx(controller, handler, { userId: 'agent:run-1:job-1' }),
    );

    expect(result).toBe(true);
    expect(authzAudit.recordDenial).toHaveBeenCalledWith(
      expect.objectContaining({
        actorId: 'agent:run-1:job-1',
        requiredPermission: 'skills:update',
        enforcementMode: 'audit',
      }),
    );
  });
});
