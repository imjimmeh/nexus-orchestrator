...
function useWorkflowRunHistory(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return api.getWorkflowRunEvents(runId);
    },
    enabled: !!runId,
...
function useWorkflowRunTelemetryAuth(runId: string | undefined) {
  return useQuery({
    queryKey: queryKeys.workflowRuns.telemetryAuth(runId ?? ""),
    queryFn: async () => {
      if (!runId) {
        return null;
      }
      return api.getWorkflowRunTelemetryAuth(runId);
    },
    enabled: !!runId,
  });
}
...
export function useWorkflowRunTelemetry(runId?: string) {
  ...
  useEffect(() => {
    setEvents([]);
    setConnectionState(runId ? "connecting" : "idle");
  }, [runId]);
  ...
  const {
    ...
    error: historyError,
  } = useWorkflowRunHistory(runId, connectionState);

  const {
    data: telemetryAuth,
    isLoading: isLoadingAuth,
    error: authError,
  } = useWorkflowRunTelemetryAuth(runId);
  ...
  useEffect(() => {
    if (!runId || connectionState === "connected") {
      return;
    }
  ...
  }, [runId, connectionState, history, isLoadingHistory]);

  useEffect(() => {
    if (!runId || !telemetryAuth?.token || !telemetryAuth.wsUrl) {
      return;
    }
  ...
  }, [runId, telemetryAuth?.token, telemetryAuth?.wsUrl]);
