 ...
 *
 * These tests cover the acceptance criteria defined in EPIC-192 for
 * `capability.endpoint` invocation including:
...
// Type definitions matching the expected plugin capability endpoint types
...
// This mirrors the expected behavior from the design doc
class MockPluginCapabilityEndpointInvocationService {
  ...
  async invoke(request: {
    ...
    if (
      ...
      !this.validateInputSchema(endpoint.outputSchema, runtimeResult.output)
    ) {
      return {
        ok: false,
        error: {
    ...
    it("returns safe error when caller lacks required permission", async () => {
      ...
      const policyService = {
        decideCapabilityEndpointInvocation: vi.fn().mockReturnValue({
          ...
          message:
            "Required permission internal_capability:admin.write is not granted.",
        } satisfies PolicyDecision),
      };
      const registryEntries = {
        findByPluginIdAndVersion: vi.fn().mockResolvedValue({
          ...
          enabled: true,
          requested_permissions: [
            { kind: "internal_capability", capabilities: ["admin.write"] },
          ],
